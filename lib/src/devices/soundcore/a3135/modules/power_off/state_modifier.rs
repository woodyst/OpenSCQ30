...
where
    T: Has<a3135::structures::PowerOffPending> + Clone + Send + Sync,
{
    async fn move_to_state(
        ...
        if *target_state.get() == a3135::structures::PowerOffPending(true) {
            self.packet_io
                .send_with_response(&a3135::packets::outbound::power_off())
                .await?;
            state_sender.send_modify(|state| {
                *state.get_mut() = a3135::structures::PowerOffPending::default();
            });
        }
        Ok(())
    }
